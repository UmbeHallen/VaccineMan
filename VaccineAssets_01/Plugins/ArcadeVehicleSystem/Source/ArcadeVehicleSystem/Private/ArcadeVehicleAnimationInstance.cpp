...
void UArcadeVehicleAnimationInstance::CalculateWheelsRotation(float DeltaTime)
{
	...
	if (bShouldRotateWheels && vehicleSettings.Engine.CurrentSpeed != 0.f)
	{
		/* Calculate speed absolute. */
		const float currentSpeedAbsolute = FMath::Abs(vehicleSettings.Engine.CurrentSpeed);
...
void UArcadeVehicleAnimationInstance::CalculateTilt(float DeltaTime)
{
	...
	if (bShouldResetTilt && FMath::Abs(Settings.Tilt.CurrentTilt) < 0.1f)
	{
		...
		float speedRatio = FMath::Clamp(FMath::Abs(vehicleSettings.Engine.CurrentSpeed / Settings.Tilt.RuntimeTilt.TiltDampingSpeed), 0.f, 1.f);
...
void UArcadeVehicleAnimationInstance::CalculateRoll(float DeltaTime)
{
	...
	float wheelSlip = FMath::Clamp((vehicleSettings.Physics.LocalLinearVelocity.Y * KMH_MULTIPLIER) / Settings.Roll.RollDampingSpeed, -1.f, 1.f);
